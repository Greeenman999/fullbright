...
abstract class ModPlatformPlugin @Inject constructor() : Plugin<Project> {
	override fun apply(project: Project) = with(project) {
		...
		val needsRemapping = inferredLoader == "fabric" && stonecutter.eval(stonecutter.current.version, "<26.1")

		val extension = extensions.create("platform", ModPlatformExtension::class.java).apply {
			loader.convention(inferredLoader)
			jarTask.convention(if (needsRemapping) "remapJar" else "jar")
			sourcesJarTask.convention(if (needsRemapping) "remapSourcesJar" else "sourcesJar")
		}
